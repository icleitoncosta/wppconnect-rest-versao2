...
export class GroupsService {
    ...
    public async get(req: RequestEx, groupId: string): Promise<ReturnGroup | Error> {
        try {
            const group = await req.client?.getChatById(groupId);
            console.log(group);
            return Promise.resolve({
                ...
                group: {
                    ...
                    pic: (await req.client?.getProfilePicFromServer(groupId) as ProfilePicThumbObj).eurl,
                    participants: await req.client?.getGroupMembersIds(groupId),
                    admins: await req.client?.getGroupAdmins(groupId),
                    invite: await req.client?.getGroupInviteLink(groupId) as string,
                }
            })
        } catch (error) {
            req.logger(error);
    ...
    public async manageGroup(req: RequestEx, payload: CreateGroup | EditGroup | AddParticipantGroup | DeleteParticipantGroup | DemoteParticipantGroup | PromoteParticipantGroup | RemoveLinkGroup): Promise<ReturnGroup | Error> {
        try {
            ...
            if(payload.type === "create_group") {
                data = await req.client?.createGroup(payload.create.name, payload.create.members);
                return Promise.resolve(this.returnGroupSucess(data?.gid._serialized as string, req));
            }else if(payload.type === "edit_group") {
                if(payload.edit.description) await req.client?.setGroupDescription(payload.edit.id, payload.edit.description);
                if(payload.edit.name) await req.client?.setGroupSubject(payload.edit.id, payload.edit.name);
                if(payload.edit.pic) await req.client?.setGroupIcon(payload.edit.id, payload.edit.pic);
                
                return Promise.resolve(this.returnGroupSucess(payload.edit.id, req));
            }else if(payload.type === "add_participant") {
                data = await req.client?.addParticipant(payload.add_participant.id, payload.add_participant.phones);
                return Promise.resolve(this.returnGroupSucess(payload.add_participant.id, req));
            }else if(payload.type === "remove_participant") {
                data = await req.client?.removeParticipant(payload.remove_participant.id, payload.remove_participant.phones)
                return Promise.resolve(this.returnGroupSucess(payload.remove_participant.id, req));
            }else if(payload.type === "demote_participant") {
                data = await req.client?.demoteParticipant(payload.demote.id, payload.demote.phone)
                return Promise.resolve(this.returnGroupSucess(payload.demote.id, req));
            }else if(payload.type === "promote_participant") {
                data = await req.client?.promoteParticipant(payload.promote.id, payload.promote.phone)
                return Promise.resolve(this.returnGroupSucess(payload.promote.id, req));
            }else if(payload.type === "revoke_link") {
                data = await req.client?.revokeGroupInviteLink(payload.revoke.id)
    ...
    private async returnGroupSucess(gwid: string, req: RequestEx): Promise<ReturnGroup>{
        const group = await req.client?.getChatById(gwid);
        return {
            messaging_product: "whatsapp",
            group: {
                id: gwid,
                ...
                pic: (await req.client?.getProfilePicFromServer(gwid) as ProfilePicThumbObj).eurl,
                participants: await req.client?.getGroupMembersIds(gwid),
                admins: await req.client?.getGroupAdmins(gwid),
                invite: await req.client?.getGroupInviteLink(gwid) as string,
            }
        }
    }
    ...
    public async manageMyGroup(req: RequestEx, type: "leave" | "join", groupId: string, link?: string): Promise<{sucess: true} | Error> {
        try {
            if(type === "leave") {
                await req.client?.leaveGroup(groupId)
                return Promise.resolve({ sucess: true });
            }else if(type === "join") {
                await req.client?.joinGroup(link as string)
                return Promise.resolve({ sucess: true });
            }else {
                return {
                    error: {
